...
export default function PaywallModal({ user, downloadCount, isPro, onClose, onRefresh, refreshing = false }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-5 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="paywall-title">
      <div onClick={(event) => event.stopPropagation()} className="relative w-full max-w-md rounded-3xl border border-white/10 bg-[#161616] p-6 shadow-2xl">
        <button type="button" onClick={onClose} aria-label="Close" className="absolute right-4 top-4 rounded-full p-2 text-white/50 hover:bg-white/10 hover:text-white"><X className="h-4 w-4" /></button>
        ...
        <a href="https://paystack.shop/pay/yl0xsmgy4e" target="_blank" rel="noopener noreferrer" className="mt-5 block cursor-pointer rounded-xl bg-[#00FF62] px-4 py-3 text-center text-sm font-bold text-black transition hover:bg-[#66ff9a]" onClick={(event) => event.stopPropagation()}>
