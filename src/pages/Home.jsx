    ...
    try {
      for (let i = 0; i < scripts.length; i++) {
        ...
        try {
          ...
          downloadBlob(blob, greenScreenExport ? `popword-mobile-overlay-${i + 1}.mp4` : `popword-${i + 1}.${blob.type.includes('mp4') ? 'mp4' : 'webm'}`);
          await recordTrialExport();
        } catch (e) {
    ...
    try {
      for (const a of aspects) {
        ...
        try {
          ...
          downloadBlob(blob, greenScreenExport ? `popword-mobile-overlay-${a.replace(':', 'x')}.mp4` : `popword-${a.replace(':', 'x')}.${blob.type.includes('mp4') ? 'mp4' : 'webm'}`);
          await recordTrialExport();
          completed.push(a);
        } catch (e) {
              ...
              <div className="flex max-w-full flex-wrap items-center justify-end gap-1.5 sm:gap-2">
                ...
                {!isPro && (
                  <div className="flex items-center gap-1.5">
                    <button type="button" onClick={handleUpgrade} disabled={upgradeLoading} title="Approximate USD display; checkout is charged in NGN" className="rounded-full bg-[#00FF62] px-3 py-1 text-[11px] font-bold text-black transition hover:bg-[#66ff9a] disabled:cursor-wait disabled:opacity-60">{upgradeLoading ? 'Opening...' : 'Upgrade — $5/mo'}</button>
                  </div>
                )}
